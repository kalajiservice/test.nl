...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert(`Demo modus: Inloggen met ${email}`);
  };
  ...
  return (
    <div className="min-h-screen bg-obsidian flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-[#111214] border border-steel p-8 rounded-lg">
        <h2 className="text-3xl font-black text-alabaster mb-2">Welcome back</h2>
        <p className="text-foreground/70 text-sm mb-6">Log in to your account</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-ochre text-obsidian font-bold py-3 mt-4 hover:bg-ochre/90 transition-colors">Log in</button>
        </form>
        <p className="text-sm text-foreground/70 mt-4 text-center">Don't have an account? <Link to="/register" className="text-ochre hover:underline">Create one</Link></p>
      </div>
    </div>
  );
}